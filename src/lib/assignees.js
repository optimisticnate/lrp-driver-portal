/* Shared helpers for resolving support ticket assignee aliases. */
const ALIAS_EMAIL = {
...
export function resolveEmailFromId(idOrEmail) {
...
export function formatAssigneeLabel(assignee) {
  if (!assignee) return "";
  if (assignee.displayName) return assignee.displayName;
  if (assignee.email) return assignee.email;
  if (assignee.userId) return assignee.userId;
  return "";
}
